...
{% macro simple_defer_with_source(model_name, version=version, v=version, defer=true) -%}
    ...
    #}
        ...
        {%- if target.name in var('defer_target_names', ()) -%}
            ...
            {%- if defer == true -%}
                ...
                {%- if model_name in var('defer_models_list', ('stg_erp__orders_details')) -%}

                    {# Finnaly, we use the source_name and model_name defined in the sources.yml file. #}
                    {{ return(builtins.source('defer_staging', model_name)) }}
